...
The SAM CLI builds a docker image from a Dockerfile and then installs dependencies defined in `home/requirements.txt` inside the docker image. The processed template file is saved in the `.aws-sam/build` folder.
...
```bash
api$ sam local invoke InferenceFunction --event events/event.json
```
...
api$ curl http://localhost:3000/classify_digit
```
...
```bash
api$ sam logs -n InferenceFunction --stack-name api --tail
```
