...
describe('Alerts', () => {
    ...
    it('Toggle webhook off using switch', () => {
      cy.login();
      cy.intercept('GET', `https://api.${domain}/account`).as('getAccount');
      cy.wait('@getAccount').then(({ request, response }) => {
        const auth = request.headers['authorization'];
        // Set user as beta user and set a webhook
        cy.request({
          method: 'POST',
          url: `https://api.${domain}/account`,
          headers: {Authorization: auth},
          body: {in_beta: 1, alerts: {webhook: 'https://test.com/webhook'}}
        });
      });
      cy.reload();
    ...
    it('Save and clear webhook URL', () => {
      cy.login();
      cy.intercept('GET', `https://api.${domain}/account`).as('getAccount');
      cy.wait('@getAccount').then(({ request }) => {
        const auth = request.headers['authorization'];
        // Set user as beta user
        cy.request({method: 'POST', url: `https://api.${domain}/account`, headers: {Authorization: auth}, body: {in_beta: 1, alerts: {webhook: ''}}});
      });
      cy.reload();
      ...
      cy.get('input[placeholder*="https://api"]').type(testUrl);
